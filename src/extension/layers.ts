...
export const LAYERS: Record<Layer, LayerSpec> = {
  ...
  event: {
    ...
    rules: [
      ...
      "推演时**先在对话里列出候选**（标题 + 一句话），等用户挑选 / 修改 / 自己指定；挑定后再用 novel_event_upsert 逐个落盘。不要一口气建一堆事件让用户被动接受。",
      "`stage` 用上面【阶段划分】里给出的阶段名，保持与大纲一致 —— 里程碑 8 的审查项 E1（主线脱轨）会拿这个字段去大纲里找该阶段的目标。",
      "事件有「当前描述」和「细化历史」两段：当前描述永远是最新最细的版本，每次细化把旧描述压进历史。",
      "伏笔不是特殊结构，它就是 origin=foreshadow 的事件：status=planned 表示已埋未收，done 表示已回收。非伏笔事件不得填 plantedIn / payoffExpectedAt。",
      "一个事件可以铺很多章（novel_event_link_chapter）；一章也可以挂多个事件，但必须有主事件（primary）。",
      "每次落盘后，把工具返回的变更摘要**原样**汇报给用户（同设定层的约定）。",
  ...
  write: {
    ...
    rules: [
      "你处在「写作模式」。本层已装载本章的**完整上下文包**（九段）—— 写正文时直接用它，不要再向用户索要人物、设定或前情。",
      ...
      "生成正文后调 novel_chapter_write 落盘，再调 novel_state_update 置 drafted。**状态只能靠工具改**，不要在对话里口头宣告「已进入下一步」—— 状态栏与后续闸门读的是 state.json。",
