...
| `/brainstorm <方向>` | outline / event / write | 按用户给定的方向起多 agent 头脑风暴 | 🟡 设施已交付（里程碑 4b，工具 `novel_brainstorm`），只剩命令入口 |
...
本层是「干完就走」的地方：进入时装载**完整上下文包**（九段，见 `ai.md「为什么 /write 每回合都要注入完整上下文」`），而每说一句话都会带上它。
...
**生成中途 Ctrl+C → 状态停在 `outlined`，不落盘半截正文。** 流式内容只给你在屏幕上看；落盘发生在生成结束后的那一次 `novel_chapter_write`。代价是：看到一半觉得不对而中止，那半截内容不会进文件（屏幕上还在）。这是「不落盘半截正文」的必然形状。
