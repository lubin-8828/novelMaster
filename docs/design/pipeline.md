...
**拒绝时逐条列出缺什么** —— 只说「不满足条件」对用户没有价值，他需要知道**去做什么**。
...
/done 校验通过
  ...
  ├─ ctx.newSession()（清空上下文）
  └─ 通知：「第 N 章已结章，上下文已清空。敲 /next 开始第 N+1 章。」
```

**下一章的推进在 `/next` 时做，不在 `/done` 时做。** 理由：`/done` 之后用户可能今天就停下了 —— 那时状态停在 `accepted` 是对的，它表示「第 N 章已结章」，而「第 N+1 章开始了吗」是另一件事。

所以 `/next` 接受两种前置状态：`not_started`（本章还没开始）与 `accepted`（上一章刚结章）；**后者要先推进章号**（`currentChapter + 1` + 置 `not_started`）。

### 7.3 实现落点

| 步 | 落点 |
|----|------|
| 1–9（回填动作） | **由 AI 逐条调现有工具** —— 每一步都要判断（谁出场、什么算关键事件、关系变没变），不能机械化 |
| 10（状态） | `novel_state_update({ chapterStatus: "reflowed", pendingReflow: false })` |
| 11（回填报告） | `novel_chapter_report_write({ kind: "reflow" })` |
| `/done` 闸门 | `/done` 命令（`src/extension/commands.ts`） |
| 清空上下文 | `ctx.newSession()` —— **只在命令处理器里可用**（事件处理器里调用会死锁，见 `ai.md`） |

**回填为什么不做成一个工具。** 11 步里 9 步都要判断。把它们塞进一个工具意味着要么让工具去调 LLM（那是另一个会话），要么让 LLM 把全部信息在一大个参数里给我（那是在参数里编）。**逐条调现有工具**才是对的形状：每步产出都能被用户看见、被日志记录。

**回填的顺序是硬要求**：先人物、再关系、再事件、最后摘要与索引。理由：事件细化要知道「谁做了什么」，而人物时间轴是那个信息最原始的落点 —— 反过来的话，事件细化会基于还没落盘的人物信息。

**`pendingReflow` 在流程里的位置**：用户说「我改好了」时，AI 先置 `{ chapterStatus: "user_edited", pendingReflow: true }`，回填完成后置 `{ chapterStatus: "reflowed", pendingReflow: false }`。`/done` 的闸门就是查这两个值 —— **闸门读的是 `state.json`，不是对话里的话**。
...
**冲突条目**（两个审查员对同一段落判定矛盾）→ 两条都保留，交给用户判断。理由：两个独立视角矛盾这件事本身就是高价值信号，机械取一方的做法会把它抹平。

**原文写的「标 `conflict: true`」已删除。** 理由：机械层面判不出「两条判定矛盾」—— 那需要语义理解，而引入语义判断正是「不做 LLM 汇总层」要避免的。实际发生的形态是「同一段落有多个不同类别的条目（A2 与 F1）」，那本来就都保留，不需要额外的冲突标记。

去重的范围故意取窄：**只去「同一审查员报了两次同一类别同一段落」**。审查员按类别分工（A / B·C / D·E·F·G），所以跨类别的条目本来就不同问题 —— 去掉它们等于把第二个视角的意见删了。
...
| 环节 | 落点 |
|------|------|
| 触发 | `novel_review` 工具（主会话）+ `/review` 命令（手动补跑） |
| 审查员编排 | `src/ai/review/index.ts`（复用 `src/ai/session.ts` 的只读子会话工厂） |
| 检查清单 | `src/ai/review/checklist.ts`（7 类 28 项**写死在代码**） |
| 输入构造 | `src/ai/review/input.ts`（`assemble()` 的九段 + **本章正文**） |
| 合并 | `src/ai/review/merge.ts` |
| 依据校验 | `src/data/validate.ts` |
| 报告渲染 | `src/ai/review/report.ts`，落盘走 `novel_chapter_report_write` |
| `submit_findings` | 子会话专用，由 `review/index.ts` 用闭包造出，**不注册给主会话** |

**审查员的输入比装配器多一段：本章正文。** `assemble()` 的九段里第 9 段是「本章大纲」，不含正文 —— 而正文正是审查对象。所以输入 = 装配产出 + 正文段。

**「自动审查」怎么实现。** pi 没有「工具成功后自动调另一个工具」的钩子，所以「自动」落在两处：

1. `novel_chapter_write` 成功后的返回文本里明确要求「接下来调 `novel_review`」；
2. 写作层规则里写明「生成正文后必须立即审查」。

**这是提示级而非机制级，必须说清楚**：如果模型忘了调，审查就不发生。但用户看得出来 —— 状态会停在 `drafted`（状态栏「正文已生成」）而不是 `auto_reviewed`（「已审查」）。**「忘了」是可被发现的，不是静默的**（与 `ops.md「失败时绝不静默降级」` 同一条）。
想要更硬只能把审查塞进 `novel_chapter_write` 内部，但那会让一个写文件的工具承担流程编排职责，也会把「写正文」与「跑三个子会话」绑成一次长达几分钟的工具调用。这个取舍见 `decisions.md`。

**子会话的 `cwd` 必须是主会话 cwd**，不是小说根 —— 否则 `novel_read_index` 在子会话里会报「没有打开小说」。这个坑在里程碑 4b 踩过一次，复用时别重踩。
...
| 环节 | 落点 |
|------|------|
| 触发 | `novel_deai` 工具（主会话）+ `/deai` 命令（手动补跑） |
| humanizer 注入 | `src/ai/deai/skill.ts` 读 `~/.pi/agent/skills/humanizer/SKILL.md` 的**文件内容** |
| 编排 | `src/ai/deai/index.ts` |
| 机械范围检查 | `src/ai/deai/scope.ts` |
| 报告渲染 | `src/ai/deai/report.ts`，落盘走 `novel_chapter_report_write` |
| 改后正文落盘 | 主会话调 `novel_chapter_write` |

**去 AI 味是「一个子会话」，不是「三个」。** 与审查的分工不同：审查要三个独立视角（单视角会漏），而去 AI 味是**一次改写** —— 三个人各改一遍，最后还得挑一份或合并，那只会让「哪一版是最终稿」变得说不清。所以它是一个子会话改写，产出一份全文。

**子会话依然只读。** 它只输出改后文本，**落盘由主会话完成** —— 这样「谁改的稿、改的哪一版」在主会话对话里有唯一的事实来源。

### 9.5 去 AI 味之后必须重查（与 §6 的冲突在这里理清）

**问题**：正文被改了，而「哪一版被审查过」必须确定。所以改完要重查 —— §6 已经这么写了。

**但重查什么，不能想当然。** 三种做法与各自的代价：

| 做法 | 代价 |
|------|------|
| 完整重跑三个审查员 | 报告里绝大多数条目是**重复的**（事实层的问题第一次报过、用户还没处理），而新改出来的问题被埋在重复条目里 |
| 不重查 | 用户验收的是一版**没被审过**的稿 —— §6 那条规矩当场作废 |
| **只重查「改稿可能改坏的」** | 需要一个判据：什么会被改坏 |

**采用的方案：只重查 F（逻辑与叙事）与 G（硬性规范）。**

判据是**改动的作用域**：去 AI 味只动表达层（句式、节奏、用词），事实层（设定、人物、关系、事件、时间线）不该被碰到 —— 那是提示词里的硬约束。所以：

- **F / G 必须重查**：叙事被改散、markdown 混入、人称时态漂移，都只可能在这一步发生；
- **A–E 不重查**：它们的问题第一次已经报过而用户还没处理；重复报只会把新问题埋掉。

**「不该被碰到」不等于「没被碰到」**，所以配一道**机械防线**：`scope.ts` 对比改前改后，扫出改动段落里是否出现人名、设定术语、ID、数字。命中就标「⚠ 改动涉及专有名词，请确认没有越界」。

判据是机械的（正则匹配已知人名与 ID + 数字），所以它能**明确报出「去 AI 味越界了」这件事**，而不靠第二个审查员去猜。

**复查报告必须标明覆盖范围**：标题写成「第 N 章 审查报告（改稿复查）」，头部标「本次只覆盖 F / G —— 事实层未被重查」。**不标的话，用户会以为这份报告和第一份一样全，而那是假的。**
