...
```
~/.novelmaster/                 # 可用 NOVELMASTER_HOME 环境变量覆盖
...
**状态全局化**（里程碑 11 之后的设计变更）：原先状态放**运行目录**（`<cwd>/.novelmaster/` + `<cwd>/novels/`），后果是「换个目录敲 `novelmaster` 就看到另一套书」—— 而那个惊吓是真实的：用户会以为书丢了。

改为全局后：**任何目录敲 `novelmaster` 都是同一本**。代价是「不同目录装不同书」不再可能 —— 但那本来就不是需求（单人单本）。

**全部状态在一个目录里**（配置 + 全部小说）：备份或搬移 = 复制 `~/.novelmaster/` 一个目录。“资料可读可改”不受影响 —— 里面依旧是普通 JSON 与 txt。

**测试靠 `NOVELMASTER_HOME` 隔离**：状态全局化后，测试会互写同一个 `~/.novelmaster/config.json`（之前就踩到过：并行/顺序都会互相污染）。环境变量覆盖是标准做法，而且它让「数据放哪」从不可控变成可注入。
...
**不再有「运行目录」的概念。** 上面两处原本都写着「（位于运行目录）」，已经改掉：数据位置与你在哪里启动无关。
...
**共 26 个 `novel_*` 工具定义**，其中 **23 个注册给主会话**；3 个**子会话专用**：`submit_brainstorm`（讨论）、`submit_findings`（审查）、`submit_deai`（去 AI 味）。
