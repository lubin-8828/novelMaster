...
```
<启动目录>/.novelmaster/         # 可用 NOVELMASTER_HOME 环境变量覆盖
├── config.json                 # 当前打开的小说（覆盖式）
└── novels/
    ├── 深海回声/                 # 一本小说 = 一个自包含目录
    │   └── （内部结构见 1.2）
    └── 另一个故事/
...
**数据跟着启动目录走**：从哪个目录敲 `novelmaster`，数据就在哪个目录。历史上有过一次全局化（`~/.novelmaster/`），理由是「换个目录启动就看到另一套书」的惊吓；用户实测后明确要求改回启动目录 —— 数据跟工作目录走才直观（备份 / 搬移 = 拷目录），「换目录换书」是期望语义。记录见 `decisions.md「与用户原始要求的偏差清单」`。

**全部状态在一个目录里**（配置 + 全部小说）：备份或搬移 = 复制 `.novelmaster/` 一个目录。“资料可读可改”不受影响 —— 里面依旧是普通 JSON 与 txt。

**测试靠 `NOVELMASTER_HOME` 隔离**：测试会互写同一个 config.json（之前就踩到过：并行/顺序都会互相污染）。环境变量覆盖是标准做法，而且它让「数据放哪」从不可控变成可注入。
...
**「运行目录」就是「启动目录」。** 数据位置与你在哪里启动直接相关 —— 这是用户明确要求的语义。

`slug` 由书名生成，**保留中文**（只把空白与符号折成连字符）：`深海回声` → `深海回声`，`Hello, World!` → `hello-world`。中文书名是常态，用 ASCII 化会得到一串没意义的目录名。
...
实现细节：`fsync` 用 `openSync(path, "r+")` 读写句柄，**不是只读句柄** —— Windows 上对只读句柄 `fsync` 会报 `EPERM`（系统要求句柄有写访问权才能 flush）。这些 tmp 文件都是刚写出来的，一定有写权限，所以读写句柄在两端都成立（见 `decisions.md「踩坑记录」`）。
...
| 工具 | 用途 | 关键参数 |
|------|------|----------|
| `novel_init` | 新建一本小说（建目录 + 初始文件 + 设为当前小说）。`/init` 的 AI 引导访谈收集齐信息、用户确认后由 AI 调用 | `title`, `genre[]?`, `premise` |
...
| `novel_deai` | **主会话**：去 AI 味（改写 + 落盘 + 自动复查 F/G） | 无参数（章号取自 `state.json`） |
| `submit_deai` | **仅去 AI 味子会话可见**：提交命中清单 + 改后全文 | `hits[]`, `rewritten` |
...
**共 27 个 `novel_*` 工具定义**，其中 **24 个注册给主会话**；3 个**子会话专用**：`submit_brainstorm`（讨论）、`submit_findings`（审查）、`submit_deai`（去 AI 味）。
